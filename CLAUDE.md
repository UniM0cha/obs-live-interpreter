...
- 음색: 화자 미러링(voice replication) — **음색 선택/고정 API 없음**(`speechConfig`/`voice_name` 미지원).
  "긴 침묵 후 음색이 바뀔 수 있음"은 공식 문서·모델 카드에 명시된 알려진 한계 (2026-07-02 확인,
  모델 카드: https://deepmind.google/models/model-cards/gemini-3-5-audio/ ).
- **glossary/용어집·custom vocabulary 미지원** (2026-07-02 확인). 음색 고정이 필요하면
  서버의 ElevenLabs 음색 변환 경로 사용(`server/README.md` 운영 가이드).
...
- 음색: dynamic voice adaptation(화자 톤 따라가기) — **voice 선택 파라미터·custom prompt·glossary
  전부 미지원** (2026-07-02 공식 가이드 확인). 음색 변동은 Gemini 와 동일한 구조적 한계.
...
- **"배포하자" 는 요청을 받았을 때만** main 으로 내보낸다. main 은 develop 의 배포 지점을 그대로 가리키는
  **fast-forward 미러**다 — develop 에만 커밋하고 main 에 직접 커밋하지 않으므로 `git merge develop` 이
  항상 머지 커밋 없이 FF 된다(`--no-ff`·`--ff-only` 등 플래그 불필요):
  ```bash
